...
class PhpClosure {
  ...
  var $_debug = true;
  var $_cache_dir = "";
  var $_cache_name = false;
  var $_code_url_prefix = "";
  var $_language = "ECMASCRIPT3";
  ...
  function addFromArray($arr = array()) {
  ...
  function setCacheName($str) {
    $this->_cache_name = $str;
    return $this;
  }

  function setLanguageECMA($ecma)
  {
    $this->_language = $ecma;
    return $this;
  }
   ...
   */
  function write($toPrint = true) {
    header("Content-Type: text/javascript");

    // No cache directory so just dump the output.
    if ($this->_cache_dir == "") {
      if ( $toPrint ) echo $this->_compile();

    } else {
      $cache_file = $this->_getCacheFileName();
      if ($this->_isRecompileNeeded($cache_file)) {
        $result = $this->_compile();
        $comment = "/*Script min updated last : " . date("M/d/Y H:i:s") . "*/";
        file_put_contents($cache_file, $comment . $result);
        if ( $toPrint ) echo $result;
      } else {
        ...
        if (@strtotime(@$_SERVER['HTTP_IF_MODIFIED_SINCE']) == $cache_mtime || 
          ...
          if ( $toPrint ) echo file_get_contents($cache_file);
        }
      }
    }

    if ( !$toPrint ) header("Content-Type: text/html; charset=utf-8");
  }
  ...
  function _getCacheFileName() {
    return $this->_cache_dir . (($this->_cache_name === false) ? $this->_getHash() : $this->_cache_name)  . ".js";
  }
